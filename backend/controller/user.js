...
// Create user (Improve added an email design 10/27/2024)
router.post("/create-user", async (req, res, next) => {
  try {
    ...
    // Attempt to upload avatar to Cloudinary
    let myCloud;
    try {
    ...
    try {
      await sendMail({
        ...
        message: ActivationContent({ userName: user.name, activationUrl }), // Generate HTML content here
      });
      res.status(201).json({
        success: true,
